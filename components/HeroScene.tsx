...
export default function HeroScene() {
  return (
    <section id="hero" className="relative h-screen w-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-end justify-center pb-16 sm:items-center sm:pb-0">
        <SectionReveal className="pointer-events-auto px-6 text-center">
          <p className="mb-3 text-xs tracking-[0.35em] text-cyan-300/80 uppercase">
            Premium 3D Portfolio
          </p>
          <h1 className="text-4xl font-bold text-cyan-300 drop-shadow-[0_0_18px_rgba(34,211,238,0.55)] sm:text-6xl">
            {profile.name}{" "}
            <span className="text-purple-300">| {profile.age}</span>
          </h1>
          <p className="mt-3 text-lg text-purple-300 sm:text-xl">{profile.title}</p>
          <p className="mt-2 text-sm text-green-400 sm:text-base">{profile.stack}</p>
          <motion.a
            href="#experience"
            className="mt-8 inline-flex rounded-full border border-cyan-400/40 bg-cyan-400/10 px-6 py-2 text-sm font-medium text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.25)]"
